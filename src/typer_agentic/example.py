...
def placeholder(type_name: str, choices: list[str] | None) -> str:
    if choices:
        return choices[0]
...
def option_tokens(opt: OptionInfo, value: str | None = None) -> list[str]:
    ...
    if opt.is_flag and value is None:
        return [name]
    values = [value or placeholder(opt.type, opt.choices)] * max(opt.nargs, 1)
    tokens = [name, *values]
...
def argument_tokens(arg: ArgumentInfo) -> list[str]:
    value = placeholder(arg.type, arg.choices)
